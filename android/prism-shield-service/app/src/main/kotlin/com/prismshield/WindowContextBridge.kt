...
class WindowContextBridge(private val service: AccessibilityService) {
    ...
    fun shouldCapture(nowElapsedMs: Long = SystemClock.elapsedRealtime()): Boolean {
        if (nowElapsedMs - lastCaptureElapsedMs < EMIT_THROTTLE_MS) {
            return false
        }
    ...
    fun buildInspectPayload(screenContext: JSONObject): JSONObject {
        ...
        return JSONObject()
            ...
            .put("text", buildUiPayloadText(screenContext))
    ...
    private fun buildUiPayloadText(screenContext: JSONObject): String {
        return JSONObject()
            .put("nodes", screenContext.optJSONArray("visible_nodes") ?: JSONArray())
            .toString()
    }
    ...
    private fun collectVisibleNodes(
        ...
        if (depth > maxDepth || !node.isVisibleToUser) {
            return
        }
        ...
        if (bounds.left < 0 || bounds.top < 0 || width <= 0 || height <= 0 || area < 50) {
            return
        }
    ...
    private fun describeWindowType(type: Int): String {
        return when (type) {
