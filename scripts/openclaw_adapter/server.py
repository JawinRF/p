...
def _inspect_rag_store(text: str) -> tuple[str, float, str, str]:
    ...
    document = {"content": text, "metadata": {"source": "openclaw_rag"}}
    validated = shield.validate_reads([document])
    strategy_result = shield.config and KeywordHeuristicStrategy().validate(text)

    if validated:
        return ("ALLOW", strategy_result.confidence, strategy_result.explanation or "MemShield allowed entry", "MemShield")
    return ("BLOCK", strategy_result.confidence, strategy_result.explanation or "MemShield blocked entry", "MemShield")
